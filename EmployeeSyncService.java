...
import com.cg.syscab.business.parameter.ParameterType;
...
public class EmployeeSyncService implements SyncService , Job{
	 ...
	 */
	private static final Integer jobTitleID = new Integer(108);
			...
			private Title getTitleAndAddTitleIfNotExist(BesEmp empFromBES) {
				...
				if ((null == title) && ( null != empFromBES.getTitle())){
					try{
						...
						ParameterType type = paramService.getParameterType(jobTitleID);
						Title newTitle = (Title)paramService.createNewParameter(type);
				        Integer rank = paramService.getNewRank(type, null);
				        newTitle.activate();
				        newTitle.setName(empFromBES.getTitle().trim());
				        newTitle.setRank(rank);
